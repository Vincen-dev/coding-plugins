...
## 任务 2： writing-technical-design skill 和路由
...
- **RED 失败:** 行为测试失败于 `writing-technical-design` 未出现在 `using-coding-plugins` 开发任务路由中。
- **GREEN 变更:** 新增 `writing-technical-design` skill、OpenAI metadata、模板、Claude 命名空间引用，并更新 `using-coding-plugins` 与 `writing-plans`。
...
- **GREEN 变更:** 新增 technical 模板中文结构校验、technical 规格缺口审查校验；更新 `writing-technical-design` 门禁、中文模板和现有 technical 文档。
- **GREEN 命令:** `python3 -m unittest scripts/test_preflight.py` PASS
- **REFACTOR 命令:** `python3 -m unittest scripts/test_preflight.py` PASS
