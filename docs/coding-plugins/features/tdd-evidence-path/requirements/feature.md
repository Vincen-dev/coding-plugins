...
为 TDD 证据 和 TDD 例外记录 规定稳定的仓库内落地路径，使规格、计划、测试证据可以按同一个 feature 检索和校验。
...
| 编号 | 优先级 | 需求 | 验证方式 |
| --- | --- | --- | --- |
| REQ-001 | 必须 | TDD 证据 默认保存到 `docs/coding-plugins/features/{feature}/evidence/tdd-evidence.md`。 | 文档评审 `skills/test-driven-development/SKILL.md`。 |
...
| 编号 | 场景 | 前置条件 | 操作 | 期望结果 |
| --- | --- | --- | --- | --- |
| AC-001 | 用户查找 TDD 最终落地文件 | 已有 feature | 查看 `docs/coding-plugins/features/{feature}/evidence/tdd-evidence.md` | 能找到 TDD 证据 或 Exception Record。 |
...
| ERR-003 | 文件检查 | `docs/coding-plugins/features/tdd-evidence-path/evidence/tdd-evidence.md` | Task 3 | 已覆盖 |
| AC-001 | 文件检查 | `docs/coding-plugins/features/tdd-evidence-path/evidence/tdd-evidence.md` | Task 3 | 已覆盖 |
