...
| NFR-006 | 必须 | active evidence 和 archived evidence 必须分离；主索引和 strict TDD 校验只处理 active `evidence/tdd-evidence.md`。 | 单元测试 `test_collect_tdd_evidence_files_excludes_archived_history`。 |
...
| ERR-003 | active evidence 旁存在 `evidence/archive/*.md`。 | preflight 不把 archive 当 active evidence 严格校验。 | `python3 -m unittest scripts/test_preflight.py`。 |
