...
- **RED 测试:** `skills/writing-technicals/scripts/test_validate_technicals.py`
- **RED 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`
- **RED 失败:** 测试因缺少 `validate_technicals.py` 报 `ModuleNotFoundError: No module named 'validate_technicals'`。
- **GREEN 变更:** 新增 `skills/writing-technicals/scripts/validate_technicals.py`，实现 technical 文件发现、frontmatter 列表解析、必需章节校验、MUST Spec ID 覆盖和 related metadata 路径校验。
- **GREEN 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`，9 tests OK。
- **REFACTOR 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`，9 tests OK。
- **最终验证:** `python3 skills/writing-technicals/scripts/validate_technicals.py docs/coding-plugins/features/spec-technical-quality-gates/technicals/spec-technical-quality-gates-TDD.md`，Technical design validation passed: 1 file(s)。
...
- **GREEN 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`，9 tests OK。
- **REFACTOR 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`，9 tests OK。
- **最终验证:** `python3 skills/writing-technicals/scripts/validate_technicals.py --strict --format json`，返回 ok=true、error_count=0、warning_count=0。
...
- **RED 失败:** 第一个测试因验证命令中缺少 `test_validate_technicals.py` 失败；第二个测试因 `preflight.check_technical_design_validator` 不存在报 AttributeError。
...
- **RED 失败:** 文档更新前，writing-technicals 技能没有提示独立 validator 和 strict 审计命令。
- **GREEN 变更:** 更新 `skills/writing-technicals/SKILL.md` 和 `docs/workflow-chain.md`，记录 validator 普通模式与 strict 模式用途。
- **GREEN 命令:** `python3 skills/spec-driven-development/scripts/validate_spec.py --strict docs/coding-plugins/features/technical-design-validator/requirements/technical-design-validator-PRD.md`，Spec validation passed。
- **REFACTOR 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py` 和 `python3 -m unittest scripts/test_preflight.py`，validator 13 tests OK，preflight 58 tests OK。
...
- **RED 测试:** `skills/writing-technicals/scripts/test_validate_technicals.py` and `scripts/test_preflight.py`
- **RED 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py` and `python3 -m unittest scripts/test_preflight.py`
...
- **GREEN 命令:** `python3 -m unittest skills/writing-technicals/scripts/test_validate_technicals.py`，13 tests OK；`python3 -m unittest scripts/test_preflight.py`，58 tests OK。
- **REFACTOR 命令:** `python3 skills/writing-technicals/scripts/validate_technicals.py --strict --format json`，ok=true、error_count=0、warning_count=0。
- **最终验证:** `python3 scripts/preflight.py`，Preflight passed。
