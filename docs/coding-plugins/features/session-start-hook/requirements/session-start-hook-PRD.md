...
| REQ-007 | 必须 | 注入上下文中的 TDD 证据 路径必须使用 feature-first 路径 `docs/coding-plugins/features/{feature}/evidences/{feature}-TDD-Evidence.md`。 | `tests/hooks/test-session-start.sh`。 |
...
| REQ-007 | hook 测试 | `bash tests/hooks/test-session-start.sh` | Task 2 | 已覆盖 |
...
| ERR-004 | hook 测试 | `bash tests/hooks/test-session-start.sh` | Task 2 | 已覆盖 |
