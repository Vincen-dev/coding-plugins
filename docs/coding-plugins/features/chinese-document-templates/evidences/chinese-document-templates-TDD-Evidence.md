...
- **RED 测试:** `skills/test-driven-development/scripts/test_validate_tdd_evidence.py`、`skills/writing-technical-design/scripts/test_validate_technical_design.py`、`scripts/test_preflight.py`
- **RED 命令:** `python3 -m unittest skills/test-driven-development/scripts/test_validate_tdd_evidence.py`; `python3 -m unittest skills/writing-technical-design/scripts/test_validate_technical_design.py`; `python3 -m unittest scripts/test_preflight.py`
- **RED 失败:** TDD 证据校验器仍只识别旧英文结构，technical 校验器仍要求旧英文映射表头，preflight 尚未实现 plan 和 TDD 证据模板中文检查函数。
- **GREEN 变更:** `validate_tdd_evidence.py` 改为中文 TDD 证据字段契约，`validate_technical_design.py` 改为中文 technical 映射表头，`preflight.py` 增加 plan/TDD 模板中文门禁，`docs_index.py` 生成中文索引表头。
- **GREEN 命令:** `python3 -m unittest skills/test-driven-development/scripts/test_validate_tdd_evidence.py skills/writing-technical-design/scripts/test_validate_technical_design.py` PASS；`python3 -m unittest scripts/test_docs_index.py scripts/test_preflight.py` PASS。
...
- **GREEN 变更:** `writing-plans`、`test-driven-development`、`writing-technical-design` 和 SDD 模板源头改为中文标题、表头和字段标签。
