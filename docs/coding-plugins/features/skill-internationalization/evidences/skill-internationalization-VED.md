...
- **最终验证:** `git diff --check` PASS；`node --test tests/ts/i18n-surface.test.mjs tests/ts/manifest-checks.test.mjs tests/ts/docs-index.test.mjs` PASS，10/10；`npm run preflight -- --write-index` still fails in `tests/ts/productization-cli.test.mjs` with 5 existing failures: arbitrary project roots for schema/list/doctor/inject, doctor stale Codex cache audit, `cli install` session-lock repair expectation, active Codex plugin enablement from `codex plugin list`, and Codex plugin-list timeout fallback. These are outside the skill internationalization scope and were already present before implementation.
