...
| REQ-003 | 必须 | `docs/coding-plugins/features/{feature}/specs/{file}.md` 必须和规格 metadata 的 `feature` 一致。 | 单元测试 `test_document_path_metadata_check_rejects_mismatched_spec_metadata`。 |
| REQ-004 | 必须 | `docs/coding-plugins/features/{feature}/evidence/tdd-evidence.md` 中引用的 Spec ID 必须能在同 feature 的规格文件中找到。 | 单元测试 `test_evidence_spec_id_check_rejects_unknown_ids`。 |
