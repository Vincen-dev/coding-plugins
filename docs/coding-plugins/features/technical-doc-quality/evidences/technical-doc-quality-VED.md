...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_document_metadata.mjs` 中的 `TypeScript technical validator rejects unfinished template content in TSD`
...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_document_metadata.mjs` 中的 `technical design template keeps relations in metadata and avoids table-heavy body`
- **RED 命令:** `node --test tests/ts/test_document_metadata.mjs`
...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_document_metadata.mjs` 中的 artifact sync dependency expectations；`tests/ts/test_scaffold_fixture_case.mjs` 中的 no generated old technical assertion。
- **RED 命令:** `node --test tests/ts/test_document_metadata.mjs`；`node --test tests/ts/test_scaffold_fixture_case.mjs`
...
- **GREEN 命令:** `node --test tests/ts/test_document_metadata.mjs tests/ts/test_scaffold_fixture_case.mjs tests/ts/test_agent_pressure_harness.mjs tests/ts/test_scenario_routing_contract.mjs`
...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_document_metadata.mjs` 中的 artifact suffix expectations；`tests/ts/test_scaffold_fixture_case.mjs` 中的 generated TSD/no generated old technical assertions；`tests/ts/test_docs_index.mjs` 中的 TSD index row assertion。
- **RED 命令:** `node --test tests/ts/test_document_metadata.mjs`；`node --test tests/ts/test_scaffold_fixture_case.mjs`；`node --test tests/ts/test_docs_index.mjs`
...
- **GREEN 命令:** `node --test tests/ts/test_document_metadata.mjs tests/ts/test_scaffold_fixture_case.mjs tests/ts/test_docs_index.mjs tests/ts/test_scenario_routing_contract.mjs tests/ts/test_agent_pressure_harness.mjs tests/ts/test_document_contract_migration.mjs`
- **REFACTOR 命令:** `npm run typecheck`
- **最终验证:** `NPM_CONFIG_CACHE=/private/tmp/codex-npm-cache npm run preflight -- --write-index` PASS。
...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_document_metadata.mjs` 中的 artifact suffix/sync expectations；`tests/ts/test_scaffold_fixture_case.mjs` 中的 generated TVD/TED/VED assertions；`tests/ts/test_docs_index.mjs` 中的 TVD index assertion。
- **RED 命令:** `node --test tests/ts/test_document_metadata.mjs tests/ts/test_scaffold_fixture_case.mjs tests/ts/test_docs_index.mjs`
...
- **测试类型:** `contract`
- **RED 测试:** `tests/ts/test_skill_document_contract.mjs`
- **RED 命令:** `node --test tests/ts/test_skill_document_contract.mjs`
...
- **GREEN 变更:** 清理 skill 文本中的旧契约说明，只保留 `related_docs` 和当前 `PRD/TSD/TVD/TED/VED` 链路；将 `test_skill_document_contract.mjs` 接入 preflight。
- **GREEN 命令:** `node --test tests/ts/test_skill_document_contract.mjs`
- **REFACTOR 命令:** `NPM_CONFIG_CACHE=/private/tmp/codex-npm-cache npm run preflight`
- **最终验证:** `NPM_CONFIG_CACHE=/private/tmp/codex-npm-cache npm run preflight` PASS，包含 `tests/ts/test_skill_document_contract.mjs` PASS。
