...
| REQ-002 | 必须 | Plan frontmatter 的 `feature` 必须与 `features/document-metadata/plans/implementation.md` 这类 feature-first 分层路径一致。 | 单元测试 `test_plan_metadata_check_rejects_mismatched_path_metadata`。 |
