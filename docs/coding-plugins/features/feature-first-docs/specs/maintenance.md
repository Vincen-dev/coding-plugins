...
| 实现计划 | `docs/coding-plugins/features/feature-first-docs/plans/implementation.md` |
...
| 编号 | 非目标 |
| --- | --- |
| NON-001 | 不保留旧 `specs/`、`technical/`、`plans/`、`evidence/` 目录作为活跃文档路径。 |
...
| NFR-002 | 必须 | 规格必须保存到 `docs/coding-plugins/features/{feature}/specs/{spec-kind}.md`。 | 单元测试和 `python3 scripts/preflight.py`。 |
| NFR-003 | 必须 | 技术设计必须保存到 `docs/coding-plugins/features/{feature}/technical/technical-design.md`。 | 单元测试 `test_collect_technical_design_files_uses_feature_first_technical_subdir`。 |
| NFR-004 | 必须 | 实现计划必须保存到 `docs/coding-plugins/features/{feature}/plans/implementation.md`。 | 单元测试 `test_collect_plan_files_uses_feature_first_plans_subdir`。 |
| NFR-005 | 必须 | TDD 证据 必须保存到 `docs/coding-plugins/features/{feature}/evidence/tdd-evidence.md`。 | 单元测试 `test_collect_tdd_evidence_files_uses_feature_first_path`。 |
