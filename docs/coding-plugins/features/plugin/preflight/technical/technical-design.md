...
| --- | --- | --- | --- | --- | --- | --- |
| REQ-001 | preflight 命令运行 SDD 校验器、TDD 证据 校验器和 preflight 逻辑的仓库单测。 | `scripts/preflight.py`：提供静态检查、索引生成、验证命令构建和主入口<br>`scripts/test_preflight.py`：覆盖 manifest、旧入口残留、索引、metadata、release 和命令构建规则 | TD-001 | `scripts/preflight.py`<br>`scripts/test_preflight.py` | 追踪矩阵中的单测命令。 | `docs/coding-plugins/features/plugin/preflight/evidence/tdd-evidence.md` |
...
TDD 证据 记录在 `docs/coding-plugins/features/plugin/preflight/evidence/tdd-evidence.md`。本轮为历史文档回填，不改变 preflight 行为，因此使用 TDD 例外记录 记录替代验证。
