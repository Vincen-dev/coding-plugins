...
| REQ-002 | 必须 | feature README 必须包含 frontmatter，并声明 `title`、`status`、`area`、`capability`、`updated` 和 `tags`。 | 单元测试 `test_feature_readme_metadata_contract_rejects_missing_frontmatter`。 |
...
| REQ-004 | 必须 | TDD Evidence 必须包含 frontmatter，并声明 `title`、`status`、`area`、`capability`、`created`、`updated`。 | 单元测试 `test_document_path_metadata_check_rejects_missing_evidence_metadata`。 |
...
| ERR-002 | README frontmatter 的 `area` 或 `capability` 与路径不一致。 | preflight 失败并指出 README metadata 与路径不一致。 | 单元测试和 preflight。 |
