...
| 编号 | 优先级 | 需求 | 验证方式 |
| --- | --- | --- | --- |
| REQ-001 | 必须 | TDD Evidence 默认保存到 `docs/coding-plugins/evidence/{area}/{capability}/tdd-evidence.md`。 | 文档评审 `skills/test-driven-development/SKILL.md`。 |
...
| REQ-005 | 必须 | preflight 必须自动严格校验 `docs/coding-plugins/evidence/**/*.md` 下的证据文件。 | 单测 `test_collect_tdd_evidence_files_uses_default_docs_path` 和 preflight 命令。 |

## 错误和边界情况

| 编号 | 条件 | 期望行为 | 验证方式 |
| --- | --- | --- | --- |
| ERR-001 | 项目没有 `docs/coding-plugins/evidence` 目录。 | preflight 跳过 evidence 文件校验，不报错。 | 单测或命令行为评审。 |
...
| 编号 | 场景 | 前置条件 | 操作 | 期望结果 |
| --- | --- | --- | --- | --- |
| AC-001 | 用户查找 TDD 最终落地文件 | 已有 area 和 capability | 查看 `docs/coding-plugins/evidence/{area}/{capability}/tdd-evidence.md` | 能找到 TDD Evidence 或 Exception Record。 |
...
| ERR-003 | 文件检查 | `docs/coding-plugins/evidence/plugin/tdd-evidence-path/tdd-evidence.md` | Task 3 | 已覆盖 |
| AC-001 | 文件检查 | `docs/coding-plugins/evidence/plugin/tdd-evidence-path/tdd-evidence.md` | Task 3 | 已覆盖 |
