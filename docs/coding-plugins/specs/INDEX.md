...
- 每次创建、移动、批准、废弃或拆分规格，都更新本索引。
- `Area` 和 `Capability` 必须和规格路径一致。
- `Type` 使用 `feature`、`api-contract`、`schema`、`state-machine`、`acceptance`、`maintenance`。
- `Status` 使用 `draft`、`approved`、`superseded`。
- `Tags` 写检索词，不写日期；日期放 `Updated` 和规格 metadata。
