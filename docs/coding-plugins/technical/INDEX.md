# Technical Design Index

本索引用于按 `Area` 和 `Capability` 检索独立技术设计文档。新增、移动或废弃 `technical-design.md` 时同步更新本文件和 `docs/coding-plugins/INDEX.md`。

| Area | Capability | Technical | Tags | Updated |
| --- | --- | --- | --- | --- |
| plugin | technical-design-artifacts | `docs/coding-plugins/technical/plugin/technical-design-artifacts/technical-design.md` | technical-design, architecture, traceability, workflow | 2026-06-26 |

Rules:

- `Area` 和 `Capability` 必须和技术设计路径一致。
- `Technical` 指向默认技术设计 `docs/coding-plugins/technical/<area>/<capability>/technical-design.md`。
- `Tags` 写检索词，不写日期；日期放 `Updated`。
