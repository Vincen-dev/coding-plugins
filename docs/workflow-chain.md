...
9. 提交必须使用中文 Conventional Commit，在 footer 添加本人作者署名，且禁止 AI 作者。
10. 最后做分支收尾和集成选择。
...
- footer 必须包含用户本人作者署名：`作者: <user.name> <user.email>`。
...
```text
用户要求提交或完成阶段选择提交 -> git-commit -> 检查 diff/status/作者身份/敏感文件 -> 中文提交并添加作者署名 footer -> 验证最新提交
```
...
3. **提交身份必须保持用户本人。** 如果 Git 配置缺失或疑似 AI/机器人身份，必须停止并询问用户；提交 footer 中的作者署名也必须和 Git Author 一致。
