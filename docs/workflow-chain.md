...
| 16 | 提交 | `git-commit` | 完成后必经提交，用户选择语言的 Conventional Commit，`Authored-by` footer，无 AI 作者 |
...
`git-commit` 负责创建提交。它参考 Conventional Commits 的类型体系，但提交说明使用中文还是英文由用户决定；用户未指定语言时必须先询问：
...
```text
用户要求提交或完成阶段选择提交 -> git-commit -> 确认提交语言 -> 检查 diff/status/作者身份/敏感文件 -> 添加 Authored-by 署名 footer -> 验证最新提交
```
