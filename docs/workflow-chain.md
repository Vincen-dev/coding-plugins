...
| 2 | 直接意图处理 | `requesting-code-review`, `receiving-code-review`, `verification-before-completion`, `using-git-commit`, `finishing-a-development-branch`, `writing-skills`, `using-git-worktrees`, `dispatching-parallel-agents` | 直接完成查询、评审、验证、提交、收尾、隔离或维护任务 |
...
flowchart TD
  ...
  C -->|提交/commit| COMMIT["using-git-commit"]
  ...
  DEV_KIND -->|已有技术方案| TEST_CASES["writing-test-cases"]
  DEV_KIND -->|已有测试用例| PLAN["writing-plans"]
  DEV_KIND -->|已有 TED| WORKTREE
  ...
  COMMIT_CHECK --> COMMIT_MSG["用户选择语言 + Authored-by footer"]
...
flowchart TD
  ...
  C -->|提交/commit| Z["using-git-commit"]
  ...
  D0 -->|已有技术方案| TC["writing-test-cases"]
...
flowchart TD
  ...
  TD --> H["写 technicals/<doc-id>-TSD.md"]
  H --> TC["writing-test-cases"]
  TC --> I["写 test-cases/<doc-id>-TVD.md"]
  I --> PLAN["writing-plans"]
  PLAN --> J["写 plans/<doc-id>-TED.md"]
  J --> K["引用 技术方案来源和测试用例来源"]
  K --> L["TED 自审或任务执行评审"]
  L --> M["进入执行场景"]
```
...
```mermaid
flowchart TD
  A["已有 TED"] --> B["using-git-worktrees"]
...
flowchart TD
  ...
  B --> C["验证测试"]
  C --> D{"有未提交变更？"}
  D -->|是，询问用户是否提交| E["using-git-commit"]
  D -->|否或暂不提交| I["检测分支/worktree/base"]
  E --> F["检查 diff、作者身份、敏感文件"]
  F --> G["用户选择语言 + Authored-by footer"]
  G --> H["验证最新提交"]
  H --> I
  I --> J{"用户选择集成方式"}
...
```mermaid
flowchart TD
  A["用户要求 commit/提交"] --> B["using-git-commit"]
  B --> C["检查 status 和 diff"]
  C --> D["检查作者身份"]
  D --> E["检查敏感文件"]
  E --> F["按逻辑暂存"]
  F --> G["用户选择语言的 Conventional Commit"]
...
| DP-1 | 需求批准 | PRD 完成后、TSD 技术方案文档前 |
| DP-2 | 技术方案批准 | TSD 技术方案文档完成后、TVD 前 |
...
`brainstorming` -> 用户确认后进入 `spec-driven-development`；构思阶段本身不创建 PRD/TSD/TVD/TED/VED

### 新需求

`spec-driven-development` -> `document-metadata` -> `writing-requirements` -> `writing-technicals` -> `writing-test-cases` -> `writing-plans` -> `using-git-worktrees` -> `test-driven-development` -> `verification-before-completion` -> `using-git-commit`
...
`using-git-commit` -> 检查 diff -> `Authored-by` -> 验证最新提交

### 完成收尾

`verification-before-completion` -> `using-git-commit` -> `finishing-a-development-branch`
...
Claude Code 侧使用 `.claude-plugin/plugin.json` 识别插件。技能以 `/coding-plugins:<skill-name>` 命名空间出现，例如 `/coding-plugins:using-coding-plugins` 和 `/coding-plugins:using-git-commit`。Claude 工具名可直接使用；平台注意事项见 [docs/claude-code-usage.md](claude-code-usage.md)。

### SDD 编排层

`brainstorming` 处理方案讨论、头脑风暴、产品方向不清和是否值得做这类 SDD 前置问题。它只输出问题定义、方案对比、推荐路径和是否进入 SDD 的判断，不创建 README、PRD、TSD、TVD、TED 或 VED。
...
- 技术方案：`writing-technicals` 负责。
- 测试用例：`writing-test-cases` 负责。
- TED 任务执行文档：`writing-plans` 负责。
- TDD 证据：`test-driven-development` 负责。
- 全局索引：`docs/coding-plugins/INDEX.md`。

硬门禁：PRD 确认前不得进入 TSD 技术方案文档；TSD 技术方案文档和 TVD 确认前不得进入 TED；不得写代码、搭脚手架或调用实现技能。

### 需求文档层

`writing-requirements` 负责把功能、接口、schema、状态机、验收或维护约束写成 PRD 需求文档。它只定义“要什么”和“如何验收”，不写技术方案、测试用例步骤或实现任务。
...
- Traceability Matrix 初稿只记录验证方式和验证证据，不写 TED 任务编号或测试步骤。
...
`writing-technicals` 把批准需求文档转成一份独立 TSD 技术方案文档。TSD 负责关键决策、影响组件、数据流、接口落地、兼容策略、测试策略、风险缓解、模块级实现拆解、接口签名、数据结构、迁移步骤和实现顺序约束。TSD 不负责逐步任务清单，也不能补写或重定义需求。

技术文档阶段必须先在 TSD 中完成 `## 规格缺口审查`。如果发现未覆盖需求、验收标准不清、新增外部行为、错误边界或兼容要求不清，停止 technical，回到 `spec-driven-development` 更新 spec、重新校验并取得确认，再继续 technical。preflight 会校验 TSD 文档包含规格缺口审查，并拦截未处理、待处理、需澄清、不清楚或待确认的缺口。

技术方案还必须完成 `## 规格到方案映射` 和 `## 无需技术方案的规格`。同一 `doc_id` 链路下 approved spec 中的每个 MUST Spec ID，都要出现在映射表里；映射表使用 7 列：`规格 ID`、`规格摘要`、`技术落点`、`关键决策 ID`、`影响文件/符号`、`验证命令`、`证据`。确实无需技术方案的，必须在豁免表中写明原因。preflight 会从同一 `doc_id` 的 approved spec 反向提取 MUST ID，拦截 technical 未覆盖或未豁免的规格。

技术方案的关键决策必须使用 `TD-001`、`TD-002` 这类稳定 ID。映射表中的 `关键决策 ID` 必须能在 `## 关键决策` 表中找到，避免规格到技术落点之间只留下泛化描述。
...
当同一 `doc_id` 已存在 PRD、TVD、TED 或 VED 时，TSD frontmatter 应维护统一的 `related_docs`。工具会根据路径后缀 `-PRD/-TSD/-TVD/-TED/-VED` 推导文档类型。这些路径用于把需求契约、技术方案、测试用例、TED 任务执行文档和验证证据连成可检索链路。

文档变更必须沿 metadata 关系向下游同步：`PRD -> TSD -> TVD -> TED -> VED`。如果上游文档的 `updated` 晚于下游，说明下游还没有完成同步评审；preflight 会失败。下游正文确实无需变更时，也要更新下游 `updated`，表示已确认不受影响。
...
```text
coding-plugins validate-technicals docs/coding-plugins/features/<feature-name>/technicals/<doc-id>-TSD.md
```
...
`writing-test-cases` 把批准需求文档和 TSD 技术方案文档转成测试用例文档。它负责 Spec ID 到测试用例的映射、测试层级、前置条件、步骤、断言、测试数据和证据目标。
...
测试用例文档不记录实际 RED/GREEN/REFACTOR 输出；实际执行证据仍由 `test-driven-development` 写入 `evidences/<doc-id>-VED.md`。

### TED 任务执行层

`writing-plans` 把 PRD、TSD 技术方案文档和 TVD 转成 `Task Execution Document (TED)`。TED 的中文定位是任务执行文档，负责把已确认的需求、技术约束和测试用例拆成可执行任务；它不复述完整技术方案，也不重定义需求。
...
- `## 任务总览`：每行一个 `TASK-001` 任务，标明覆盖规格、验证方式和 VED 目标。
- `## 任务标题（TASK-001 / REQ-001）`：逐任务展开目标、前提、修改范围、执行步骤、验证方式和 VED 记录要求。
- `## 完成检查`：确认每个 MUST Spec ID、测试命令和 VED 字段都有执行落点。

完整技术方案以 TSD 为准；测试设计以 TVD 为准；路径关系以 frontmatter metadata 和 INDEX 为准。
...
- 每个任务派发一个新的实现子代理。
- 子代理不能自己读取完整 TED，主代理应提供完整任务文本和必要上下文。
...
| `context_compression` | 记录上下文压缩边界：当前 TED 任务章节、workflow brief、`source_hash`、must-read/may-skip 数量。 |

### 提交层

`using-git-commit` 负责创建提交。它参考 Conventional Commits 的类型体系，但提交说明使用中文还是英文由用户决定；用户未指定语言时必须先询问：
...
- description、body、footer 的说明文字必须和用户选择的语言一致。
...
1. 验证测试。
2. 检查是否有未提交变更。
3. 如有未提交变更，进入 `using-git-commit` 按逻辑分批提交；只有用户明确要求不提交时才跳过。
...
查找某个功能时，先按 `Feature` 或 `标签` 查总索引，再进入对应需求文档、技术方案文档、测试用例、TED 或 VED 证据文件。新增、移动或删除相关产物时必须运行 `npm run preflight -- --write-index` 重新生成总索引；`src/cli/preflight.ts` 会校验索引文本和真实 feature-first 文件树完全一致。
...
```text
writing-plans -> executing-plans -> requesting-code-review -> verification-before-completion -> using-git-commit -> finishing-a-development-branch
```
...
```text
用户要求提交或完成阶段选择提交 -> using-git-commit -> 确认提交语言 -> 检查 diff/status/作者身份/敏感文件 -> 添加 Authored-by 署名 footer -> 验证最新提交
```
...
1. **完成后默认提交。** 完成验证后必须进入 `using-git-commit`，按逻辑分批提交；只有用户明确要求不提交或仓库流程阻塞时才跳过。
2. **worktree 顺序应明确。** 推荐维护为统一顺序：先需求文档，再技术方案文档，再测试用例，再 TED 任务执行文档，再创建或确认隔离工作区，再执行。
...
2. **建立 prompt/token 预算回归。** `tests/ts/agent-pressure-harness.test.mjs` 覆盖 `implementer`、`spec-reviewer`、`code-quality-reviewer`、`all` 四种输出模式，检查 prompt 数量、字符预算和输入占位符泄漏。
...
5. **沉淀上下文压缩策略。** `context_compression` 明确记录当前任务章节、workflow brief、`source_hash`、must-read/may-skip 边界，避免子代理重复消费完整 PRD/TSD/TVD 链路。
