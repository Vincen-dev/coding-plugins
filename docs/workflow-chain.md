...
查找某个功能时，先按 `Feature` 或 `标签` 查总索引，再进入对应需求文档、技术设计、技术实现、测试用例、IPD 或 TDD 证据文件。新增、移动或删除相关产物时必须运行 `python3 scripts/preflight.py --write-index` 重新生成总索引；`scripts/preflight.py` 会校验索引文本和真实 feature-first 文件树完全一致。
...
1. **按 `--kind` 懒生成 prompt payload。** 当前 CLI `--json` 已按 `--kind` 过滤输出，但 `build_prompts()` 仍会一次构造实现、规格评审和代码质量评审三个 prompt。后续可把 prompt 构造拆成按需生成，降低单一子代理场景下的内存、CPU 和 token 泄漏面。
2. **建立 prompt/token 预算回归。** 在 `scripts/test_subagent_prompt_builder.py` 或 agent pressure harness 中加入 payload 字符数、prompt 数量和占位符泄漏断言，覆盖 `implementer`、`spec-reviewer`、`code-quality-reviewer`、`all` 四种输出模式。
3. **把子代理成本策略落成机器检查。** 将“机械实现用低成本模型、集成/评审用标准或强模型”的规则转成可审计字段，例如任务复杂度、文件数、风险等级和推荐模型档位，避免每次人工判断。
4. **扩展真实 agent 压力测试。** 增加子代理模式下的正向执行案例：实现子代理只拿当前 TASK 章节，规格评审只拿实现报告和任务契约，代码质量评审只拿实现报告和 git range，并保存 command log / transcript hash。
5. **沉淀上下文压缩策略。** 对重复出现的执行锁定区、任务上下文和评审输入建立摘要/引用边界，优先传 `source_hash`、prompt hash 和必要摘录，避免每个子代理重复吃完整链路上下文。
