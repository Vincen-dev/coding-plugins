...
1. 验证测试。
2. 检测普通仓库、worktree 或 detached HEAD。
3. 判断 base branch。
4. 给出 merge、PR、保留、丢弃选项。
5. 按用户选择执行。
6. 对本流程创建的 worktree 做清理。
...
1. **提交动作需要用户或仓库流程允许。** 原版流程多处要求写文档后 commit、每任务 commit。实际使用时应遵守用户指令和仓库规则，不应在用户未授权时擅自提交。
2. **worktree 顺序应明确。** README 中描述为设计批准后使用 worktree；`writing-plans` 中描述为执行阶段使用 worktree。推荐维护为统一顺序：先设计，再计划，再创建或确认隔离工作区，再执行。

## 推荐后续改进

1. 将所有“必须 commit”的措辞调整为“如果用户允许或仓库流程要求，则 commit”。
2. 在 `brainstorming`、`writing-plans` 和 README 中统一标准顺序。
3. 明确 `using-superpowers` 是兼容入口，避免和 `using-coding-plugins` 形成重复主入口。
4. 若准备作为个人 marketplace 插件安装，再补 marketplace 注册和安装说明。
