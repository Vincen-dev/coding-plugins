...
Claude Code 侧使用 `.claude-plugin/plugin.json` 识别插件。技能以 `/coding-plugins:<skill-name>` 命名空间出现，例如 `/coding-plugins:using-coding-plugins` 和 `/coding-plugins:git-commit`。Claude Code 是原 Superpowers 的主要目标环境之一，Claude 工具名可直接使用；平台注意事项见 [docs/claude-code-usage.md](claude-code-usage.md)。
...
当前流程继承了原 Superpowers 的强纪律，但在跨 Codex/Claude Code 环境中有几个点需要维护者特别注意：
