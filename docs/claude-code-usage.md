...
方案讨论、头脑风暴、产品方向不清或先分析不落地时，使用 `/coding-plugins:brainstorming`。确认进入落地后，由 `/coding-plugins:using-coding-plugins` 按风险选择 Inspect、Quick、Standard、Governed 或 Critical；需要持久化上下文时再使用 `/coding-plugins:change-capsule`。
...
请先读取并遵守 Coding Plugins 入口规则，按风险选择最小安全工作流。实现前写清 Outcome、Boundary、Verification 并建立测试先行证据；完成前运行和读取最新验证。按需使用 Change Capsule、调试、评审、提交或分支收尾技能。
```
