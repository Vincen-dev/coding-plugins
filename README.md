...
| Start Contract | 用户级入口先运行 Coding Plugins CLI 的 `start`，再选择 skill，避免纯对话判断 |
...
入口永远从 Coding Plugins CLI 的 `start` 开始。Codex 插件安装后，SessionStart 会注入本次可用的 `CP_CLI` fallback；用户不需要先把 `coding-plugins` 安装到全局 PATH。

安装后告诉 Agent：

```bash
${CP_CLI} start --intent "我要实现 <功能>" --root .
```
...
```bash
${CP_CLI} start --root . --feature <feature> --doc-id <doc-id> --intent "开始执行"
```
...
```bash
${CP_CLI} start --root . --feature <feature> --doc-id <doc-id> --intent "继续执行"
