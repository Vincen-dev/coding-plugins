...
| 提交前门禁 | `coding-plugins commit-guard --language zh --author-name <name> --author-email <email> --branch <branch> --changed-files <paths> --feature <feature> --doc-id <doc-id> --json` |
...
| 18 | `using-git-commit` | 提交 | 用户选择语言的 Conventional Commit、作者身份和敏感文件检查 |
...
提交是工作流边界。`using-git-commit` 会检查 diff、作者身份、敏感文件和 `Authored-by` footer，避免把 AI 作者、秘密信息或多个无关变更混进一次提交。
...
${CP_CLI} commit-guard \
  ...
  --changed-files "$(git diff --cached --name-only | paste -sd, -)" \
  --json
```
