...
| GitHub Copilot CLI | `coding-plugins install-copilot --root <project>` | 重新运行安装命令 | 删除 `.github/copilot-instructions.md` |
| OpenCode / Trae / Qoder / Trae CN | clone + symlink/copy `skills/` | `git pull`，复制安装需重新复制 | 删除 skills 目录 |
| Cursor | `coding-plugins install-cursor --root <project>` | 重新运行安装命令 | 删除 `.cursor/rules/coding-plugins.mdc` |
...
该命令会写入 `.github/copilot-instructions.md`，让 Copilot 在项目内优先使用 `coding-plugins start`、`state`、`validate` 和 `execution-contract`。默认不会覆盖已有文件；确认替换时使用 `--force`。它不会调用未验证的 `copilot plugin ...` 命令。
...
该命令会写入 `.cursor/rules/coding-plugins.mdc`。默认不会覆盖已有文件；确认替换时使用 `--force`。如需先看变更：
