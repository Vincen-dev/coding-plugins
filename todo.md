...
- [x] 拆分 skill 本体说明和门禁规则：`skills/*/SKILL.md` 只保留技能目标、触发条件、执行步骤和必要参考；工作流门禁、DP 检查、发布/提交约束等规则沉到共享 guard、CLI 或专门文档中。
...
状态说明：已创建 `skills-optimization` 正式文档链，PRD、TSD、TVD、TED 和 VED 已批准；已落地 `skill-boundary-inventory.md` 和 `tests/ts/skill-guard-boundary.test.mjs`，完成 high-risk skill 文案收敛、全量 migration action 收口和完整 `npm run preflight -- --write-index` 验证。
