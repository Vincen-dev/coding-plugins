...
import { sha256Json, sha256Text } from "./agent-pressure-ingest.ts";

type CommandLog = Record<string, unknown>;
type CasePayload = Record<string, any>;

function runLoggedCommand(command: string[], cwd: string, excerpts: { stdoutExcerpt?: string; stderrExcerpt?: string } = {}): [CommandLog, string, string] {
...
function commandLog(command: string[], cwd: string, excerpts: { stdoutExcerpt?: string; stderrExcerpt?: string } = {}): CommandLog {
  return runLoggedCommand(command, cwd, excerpts)[0];
}

function syntheticLog(command: string, cwd: string, options: { exitCode: number; stdout?: string; stderr?: string; stdoutExcerpt?: string; stderrExcerpt?: string }): CommandLog {
...
export function runIpdScenario(root: string): CasePayload {
...
export function runCommitSafetyScenario(_root: string): CasePayload {
...
export function runParallelScenario(root: string): CasePayload {
  const ok = commandLog(
    [
      "node",
      "--test",
...
export function runSubagentPromptScenario(root: string): CasePayload {
  const feature = "routing-fixture";
  const docId = "routing-login";
  const [fixtureRoot, locateLog] = findFixtureRoot(root, feature, docId);
  void locateLog;
  ...
  const promptCommands: Array<[string, string[]]> = [
    ["implementer", [process.execPath, "skills/subagent-driven-development/scripts/subagent-prompt-builder.ts", "--root", fixtureRootArg, "--feature", feature, "--doc-id", docId, "--task", "TASK-001", "--kind", "implementer", "--json"]],
    ["spec-reviewer", [process.execPath, "skills/subagent-driven-development/scripts/subagent-prompt-builder.ts", "--root", fixtureRootArg, "--feature", feature, "--doc-id", docId, "--task", "TASK-001", "--kind", "spec-reviewer", "--implementer-report", report, "--json"]],
    ["code-quality-reviewer", [process.execPath, "skills/subagent-driven-development/scripts/subagent-prompt-builder.ts", "--root", fixtureRootArg, "--feature", feature, "--doc-id", docId, "--task", "TASK-001", "--kind", "code-quality-reviewer", "--implementer-report", report, "--base-sha", "abc1234", "--head-sha", "def5678", "--json"]],
    ["all", [process.execPath, "skills/subagent-driven-development/scripts/subagent-prompt-builder.ts", "--root", fixtureRootArg, "--feature", feature, "--doc-id", docId, "--task", "TASK-001", "--kind", "all", "--implementer-report", report, "--base-sha", "abc1234", "--head-sha", "def5678", "--json"]],
  ];
  const logs: CommandLog[] = [];
  ...
  const budgetOk = Object.values(payloads).every((payload: any) =>
    Object.values(payload.prompt_budget).every((budget: any) => budget.within_budget && budget.placeholder_leaks.length === 0),
  );
  ...
  const compressionOk = Object.values(payloads).every((payload: any) => payload.context_compression.strategy === "task-section-plus-brief" && payload.context_compression.must_read_count === 1);
...
export function writeHarnessOutput(payload: Record<string, any>, outputPath: string): void {
