...
import { artifactDirectories, artifactFile } from "./document-metadata.ts";
...
function yamlList(values: string[], fallback?: string): string {
  if (values.length > 0) {
...
export function renderReadme(feature: string, title: string, status: string, updated: string, tags: string[]): string {
  return (
    "---\n" +
    `title: ${title}\n` +
    `status: ${status}\n` +
...
export function renderPrd(feature: string, docId: string, title: string, status: string, currentDate: string, tags: string[]): string {
...
export function scaffoldFeature(root: string, feature: string, title: string, options: ScaffoldFeatureOptions = {}): ScaffoldResult {
