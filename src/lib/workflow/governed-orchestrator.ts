...
import { buildResult } from "../documents/validate-tdd-evidence.ts";
import { approveDecision, approveDecisionV2, auditDecisionV2, auditDecisions, getDecisionStatus } from "./decision-state.ts";
import type { DecisionAuditResult, DecisionBundleInput, DecisionStatusResult, DecisionV2Result } from "./decision-state.ts";
import { auditCompletion } from "./completion-state.ts";
import type { CompletionSummary } from "./completion-state.ts";
import { loadActiveChange } from "./active-change.ts";
import { loadPolicyRegistry, resolvePolicyBundle } from "./policy-resolver.ts";
import type { PolicyRegistry, ResolvedPolicyBundle } from "./policy-resolver.ts";
import { auditPolicyCoverage } from "./policy-coverage.ts";
import { auditTechnicalBundle } from "./technical-approval.ts";
import { artifactPath, computeUpstreamHash, parseFrontmatter } from "./workflow-state.ts";
...
function readArtifact(root: string, feature: string, docId: string, suffix: string): string {
  const path = artifactPath(root, { feature, docId, suffix });
  return existsSync(path) ? readFileSync(path, "utf8") : "";
}

function policyIds(text: string): string[] {
  return [...new Set([...text.matchAll(/\bPOL-[A-Z0-9-]+\b/g)].map((match) => match[0]))].sort();
}

function policyIdsInSection(text: string, heading: string): string[] {
...
function repositoryKind(root: string): string {
  return existsSync(join(root, "package.json")) ? "typescript" : "generic";
}

function emptyRegistry(): PolicyRegistry {
  return { schemaVersion: 1, policies: [], skillBindings: [] };
}

function resolvePolicies(root: string): ResolvedPolicyBundle {
...
export function workflowSchema(root: string, options: { feature: string; docId: string }): WorkflowSchema {
...
function bundleFor(root: string, options: { feature: string; docId: string; id: "DP-1" | "DP-2" | "DP-3" }): {
  bundle: DecisionBundleInput;
  policies: ResolvedPolicyBundle;
} {
...
function technicalBlockers(root: string, options: { feature: string; docId: string }, policies: ResolvedPolicyBundle): string[] {
...
function approveTechnicalDocuments(root: string, options: { feature: string; docId: string }): void {
...
export function approveGovernedDecision(root: string, options: {
  feature: string;
  docId: string;
  id: "DP-1" | "DP-2" | "DP-3";
  reason: string;
...
export function auditGovernedExecution(root: string, options: { feature: string; docId: string }): DecisionAuditResult {
  if (workflowSchema(root, options) === "governed-v1") {
...
function decisionsApproved(root: string, options: { feature: string; docId: string }, schema: WorkflowSchema): boolean {
...
export function auditFormalCompletion(root: string, options: { feature: string; docId: string }): FormalCompletionResult {
