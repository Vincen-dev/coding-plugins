...
export const PACKAGE_ALLOWED_FILE_ENTRIES: readonly string[] = [
  "bin/",
  "dist/",
...
export const PACKAGE_DISALLOWED_BROAD_ENTRIES = ["src/", "docs/", "tests/"] as const;

export type PackagePolicyIssueKind =
  | "not_allowed"
  | "denied_dev_content"
...
export function requiredPackageFileEntries(): Set<string> {
  return new Set(PACKAGE_ALLOWED_FILE_ENTRIES);
}

export function auditPackageManifestEntries(entries: string[]): PackagePolicyIssue[] {
  ...
  for (const required of PACKAGE_ALLOWED_FILE_ENTRIES) {
    if (!entrySet.has(required)) {
      issues.push({
        kind: PACKAGE_REQUIRED_USER_DOCS.includes(required as (typeof PACKAGE_REQUIRED_USER_DOCS)[number])
          ? "missing_user_doc"
          : "missing_required_runtime",
        path: required,
      });
...
export function isDeniedPackagePath(path: string): boolean {
  return (
    path.startsWith("docs/coding-plugins/features/") ||
...
function isAllowedPackagePath(path: string): boolean {
  if (PACKAGE_REQUIRED_RUNTIME_FILES.includes(path as (typeof PACKAGE_REQUIRED_RUNTIME_FILES)[number])) {
    return true;
  }
...
export function auditPackedFilePaths(paths: string[]): PackagePolicyIssue[] {
