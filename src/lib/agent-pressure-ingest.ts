...
const FIXTURE_DESCRIPTION =
  "Real sub-agent pressure test results captured from Codex multi-agent runs. Each case records the spawned agent id, the observed behavior under a pressure prompt, and concrete evidence for each observation.";
...
export function caseFilename(caseId: string): string {
  return `${caseId.replace(/[^A-Za-z0-9_.-]+/g, "-").toLowerCase()}.json`;
}

function requireFields(value: Record<string, any>, required: Set<string>, label: string): void {
...
function validateCaseStatus(caseData: Record<string, any>): void {
...
function validateCaseEvidence(caseData: Record<string, any>): void {
...
export function normalizeCommandLog(command: Record<string, any>): Record<string, any> {
...
export function normalizeCase(rawCase: Record<string, any>): Record<string, any> {
...
export function normalizePayload(rawPayload: Record<string, any>): Record<string, any> {
...
function splitCaseFilenames(cases: Array<Record<string, any>>): string[] {
