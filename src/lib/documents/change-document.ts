...
export function selectArtifactProfile(options: { flow: UserFlow; multiTurn?: boolean }): ArtifactProfile {
...
function encodeRecord(record: ActiveChangeRecord): string {
  return Buffer.from(JSON.stringify(record), "utf8").toString("base64url");
}

export function parseStandardChangeDocument(text: string): ActiveChangeRecord | null {
  const match = new RegExp(`<!--\\s*${ACTIVE_CHANGE_MARKER}:([A-Za-z0-9_-]+)\\s*-->`).exec(text);
  if (!match) {
    return null;
  }
  try {
    return JSON.parse(Buffer.from(match[1], "base64url").toString("utf8")) as ActiveChangeRecord;
  } catch {
    return null;
  }
}

export function renderStandardChangeDocument(record: ActiveChangeRecord): string {
  return [
    "---",
...
export function validateStandardChangeDocument(text: string): string[] {
  const errors: string[] = [];
...
export function standardChangeDocumentPath(root: string, changeId: string): string {
  return join(root, "docs/coding-plugins/changes", changeId, "change.md");
}

export function writeStandardChangeDocument(root: string, record: ActiveChangeRecord): string {
...
export function readStandardChangeDocument(path: string): ActiveChangeRecord | null {
