#!/usr/bin/env python3
"""Stable decision-point definitions for Coding Plugins workflows."""
...
def all_decision_points() -> list[dict[str, Any]]:
    return [dict(point) for point in DECISION_POINTS]


def get_decision_point(point_id: str) -> dict[str, Any]:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="List Coding Plugins decision points.")
    parser.add_argument("--id", dest="point_id", help="Return one decision point.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload: Any = get_decision_point(args.point_id) if args.point_id else all_decision_points()
    if args.json:
        ...
        print(f"required_input: {payload['required_input']}")
        print(f"expected_output: {payload['expected_output']}")
