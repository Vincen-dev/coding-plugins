...
如果 `coding-plugins` 不在 PATH，不要把证据校验标记为不可用；先使用本地插件仓库 fallback：

```bash
node /absolute/path/to/coding-plugins/bin/coding-plugins.js validate-tdd-evidence docs/coding-plugins/features/<feature-name>/evidences/<doc-id>-VED.md
```
