...
本技能参考 Conventional Commits 思路：类型和可选 scope 保持英文规范，description、body 和 footer 说明使用用户选择的中文或英文。每次提交必须在 footer 中添加用户本人的 `Authored-by` 署名。

## 提交语言

提交信息可以使用中文或英文，必须由用户决定。

- `type` 和 `scope` 保持英文 Conventional Commit 标识，例如 `feat(commit)`、`docs`、`fix(auth)`。
- description、body、footer 中给人阅读的说明文字必须和用户选择的语言一致。
- `Authored-by`、`BREAKING CHANGE`、issue key 这类机器或规范字段名可保留英文，字段内容按用户选择的语言说明。
- 当前对话持续使用中文或英文，且用户直接要求提交时，可视为该语言偏好；如果对话语言混合、用户切换语言或你不确定，提交前先询问。
- 如果用户没有明确语言偏好，提交前先询问用户；不要自行选择中文或英文。
...
1. **提交信息语言必须遵循用户选择。**
   ...
   - description、body、footer 的说明文字使用用户指定的中文或英文。
   - 用户未指定语言且当前对话语言也不能明确判断时，先询问再提交。
...
如果用户已经明确要求中文或英文，直接使用该语言。若用户没有明确语言偏好，先询问：
...
- **type**：变更类型。
- **scope**：受影响模块，可省略。
- **description**：使用用户选择语言的一句话，祈使/动宾结构，尽量不超过 72 字符。
- **body**：使用用户选择语言，必要时说明原因、设计取舍和影响。
...
- 分析 diff 并按逻辑暂存文件
- 按你选择的中文或英文生成 Conventional Commit 信息
...
- 已检查敏感文件。
- 暂存内容是一个逻辑变更。
- 提交信息为 Conventional Commit，且语言与用户选择一致。
