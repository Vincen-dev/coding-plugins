...
6. **填写 metadata**：复制或补齐 `templates/spec-metadata.md` 字段；日期写在 `created`/`updated`，不要写进文件名。
...
路径只表达归属和类型，不表达时间。时间、状态、标签、相关代码和相关规格写入文档 metadata，并同步到 `docs/coding-plugins/specs/INDEX.md`。
