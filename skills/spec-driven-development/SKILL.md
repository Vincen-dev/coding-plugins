...
先确定 feature 要沉淀哪些落地文档，再按顺序编排 PRD 需求文档、TDD/TID 技术文档、TCD 测试用例、IPD 实现计划和 TED 证据。
...
6. **创建文档骨架**：新 feature 优先运行 `python3 skills/spec-driven-development/scripts/scaffold_feature_docs.py <feature-name> --title "<功能名称>"`，生成 README、PRD 和标准子目录；已存在时只更新目标文档。
7. **编写需求文档**：调用 `writing-requirements`，输出 `docs/coding-plugins/features/<feature-name>/requirements/<feature-name>-PRD.md`。
8. **需求校验和确认**：运行 `validate_spec.py`，按 `references/spec-review-checklist.md` 自审，并等待用户确认。
9. **编写技术文档**：需求确认后调用 `writing-technicals`，输出 `technicals/<feature-name>-TDD.md`，需要模块级落地时同时输出 `technicals/<feature-name>-TID.md`。
10. **编写测试用例**：TDD/TID 确认后调用 `writing-test-cases`，输出 `test-cases/<feature-name>-TCD.md`。
...
| PRD 需求文档 | `writing-requirements` | `docs/coding-plugins/features/<feature-name>/requirements/<feature-name>-PRD.md` | 需求、契约、验收、Spec ID |
| TDD 技术设计 | `writing-technicals` | `docs/coding-plugins/features/<feature-name>/technicals/<feature-name>-TDD.md` | 架构、关键决策、技术落点 |
| TID 技术实现 | `writing-technicals` | `docs/coding-plugins/features/<feature-name>/technicals/<feature-name>-TID.md` | 模块、接口、数据结构、迁移和实现约束 |
| 测试用例文档 | `writing-test-cases` | `docs/coding-plugins/features/<feature-name>/test-cases/<feature-name>-TCD.md` | Spec ID 到测试用例、断言、数据 |
| 实现计划 | `writing-plans` | `docs/coding-plugins/features/<feature-name>/plans/<feature-name>-IPD.md` | 任务拆分、命令、执行顺序 |
| TED 证据 | `test-driven-development` | `docs/coding-plugins/features/<feature-name>/evidences/<feature-name>-TED.md` | 实际 RED/GREEN/REFACTOR 证据 |
...
这些模板只决定 PRD 正文中需要覆盖的章节和契约维度，不决定最终文件名。一个 feature 的需求沉淀到同一个 `<feature-name>-PRD.md`；如果需求横跨多个独立 feature，拆成多个 feature root。
...
PRD 内可以组合 feature、API contract、schema、state machine、acceptance 或 maintenance 章节。不要再按规格类型创建多个独立需求文件。
...
如果同一 `features/<feature-name>/requirements/<feature-name>-PRD.md` 已存在，优先更新它；如果新需求横跨多个独立 feature，拆成多个 feature root，并在 `related_specs` 和 `INDEX.md` 中互链。
