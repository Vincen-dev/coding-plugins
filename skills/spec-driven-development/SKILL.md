...
description: 新需求、行为变更、接口契约、schema、状态机或验收标准尚未明确，且准备进入 TED 任务执行文档前使用。
---
...
先确定 feature 要沉淀哪些落地文档，再按顺序编排 PRD 需求文档、TSD 技术方案文档、TVD 测试用例、TED 任务执行文档和 VED 证据。

**核心原则：**`spec-driven-development` 是 SDD 文档链路编排入口，不直接承接所有正文写作；PRD 需求正文交给 `writing-requirements`，TSD 技术方案文档交给 `writing-technicals`，TVD 测试用例交给 `writing-test-cases`，最后才进入 `writing-plans`。
...
- 需求变更必须先改 PRD，再改 TSD、TVD、TED，最后再改实现。
...
- 已有批准过的 PRD 和 TSD：直接用 `writing-test-cases`。
- 已有批准过的 PRD、TSD 和 TVD：直接用 `writing-plans`。
...
2. **设计文档链路**：确认本 feature 需要沉淀哪些文档：README、PRD、TSD、TVD、TED、VED。
...
5. **先读 metadata**：涉及 Coding Plugins 文档关系时先使用 `document-metadata`；读取目标 feature README 和相关 PRD、TSD、TVD、TED、VED 的 frontmatter；关联关系以 `related_docs`、README `tags` 和 `docs/coding-plugins/document-contract.md` 为准。
6. **创建文档骨架**：新 feature 优先运行 `coding-plugins scaffold-feature-docs <feature-name> --title "<功能名称>"`，生成 README、PRD 和标准子目录；同一 feature 下新增第二条文档链路时加 `--doc-id <doc-id>`；已存在时只更新目标文档。
7. **编写需求文档**：调用 `writing-requirements`，输出 `docs/coding-plugins/features/<feature-name>/requirements/<doc-id>-PRD.md`。
8. **需求校验和确认**：运行 `validate-spec.ts`，按 `references/spec-review-checklist.md` 自审，并等待用户确认。
9. **编写技术文档**：需求确认后调用 `writing-technicals`，输出 `technicals/<doc-id>-TSD.md` 技术方案。
10. **编写测试用例**：TSD 确认后调用 `writing-test-cases`，输出 `test-cases/<doc-id>-TVD.md`。
11. **维护索引和关系**：新增、移动、批准、废弃或拆分文档时，运行 `npm run preflight -- --write-index` 生成 `docs/coding-plugins/INDEX.md`；新增下游 TSD/TVD/TED/VED 后必须回填 PRD 的 `related_docs` metadata。
12. **过渡计划**：PRD、TSD 和 TVD 都确认后，调用 `writing-plans`。

## 文档生成决策矩阵

先判断任务是否需要沉淀文档，再决定链路深度。不要为了形式创建文档；也不要在存在长期契约风险时跳过文档。

| 场景 | 文档决策 | 需要的文档 |
| --- | --- | --- |
| 只解释代码、读取状态、运行验证、普通 review、无行为影响的小整理 | 不创建新文档 | 不创建 PRD/TSD/TVD/TED/VED；只回报结果或使用对应技能 |
| 新功能、用户可见行为、API/SDK/CLI 契约、schema、状态机、兼容迁移、安全/性能/稳定性改造、长期回归修复 | 完整链路 | README + PRD -> TSD -> TVD -> TED -> VED |
| demo/sample、一次性验证、纯文档整理、极小配置或文案调整、已有外部验证报告且无长期实现计划 | 轻量例外 | README + PRD + VED；README 的 `## 轻量例外` 必须按 `Doc ID | 规格 ID | 证据` 声明适用范围 |
| 需求已 approved，但尚未执行实现 | 计划链路 | PRD + TSD + TVD + TED；VED 可以先是 draft 证据目标，不要求最终验证通过 |
| 实现已完成或 TSD 标记 `lifecycle_status: implemented` | 完成链路 | PRD + TSD + TVD + TED + VED，VED 必须包含最终验证通过的证据 |

决策顺序：

1. 判断是否有新需求、契约、验证口径或长期维护风险；没有则不创建新文档。
2. 如果有长期工程契约，走完整链路。
3. 如果只是轻量验证或纯文档链路，使用 doc_id 级轻量例外，不要把整个 feature 都豁免。
4. 如果同一 feature 下有多条独立需求，按 `doc_id` 拆分链路。
5. 如果需求横跨多个独立 feature，拆分 feature root，并用 `related_docs` 互链。
...
| TSD 技术方案文档 | `writing-technicals` | `docs/coding-plugins/features/<feature-name>/technicals/<doc-id>-TSD.md` | 架构、关键决策、技术落点、实现方案、测试交接 |
| 测试用例文档 | `writing-test-cases` | `docs/coding-plugins/features/<feature-name>/test-cases/<doc-id>-TVD.md` | Spec ID 到测试用例、断言、数据 |
| TED 任务执行文档 | `writing-plans` | `docs/coding-plugins/features/<feature-name>/plans/<doc-id>-TED.md` | 任务拆分、命令、执行顺序 |
| VED 证据 | `test-driven-development` | `docs/coding-plugins/features/<feature-name>/evidences/<doc-id>-VED.md` | 实际 RED/GREEN/REFACTOR 证据 |
...
如果同一 `features/<feature-name>/requirements/<doc-id>-PRD.md` 已存在，优先更新它；如果新需求属于同一 feature 模块但有独立边界，创建新的 `doc_id` 链路；如果横跨多个独立 feature，拆成多个 feature root，并在 `related_docs` 和 `INDEX.md` 中互链。
...
```bash
coding-plugins validate-spec --format json <SPEC_FILE_PATH> [<OTHER_SPEC_FILE>...]
```
