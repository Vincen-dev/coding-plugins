...
4. **检索既有规格**：先查 `docs/coding-plugins/INDEX.md`，再用 `rg` 查 area、capability、tag、Spec ID 和相关代码路径；能更新既有规格时不要新建。
5. **写结构化规格**：按选定模板输出到 `docs/coding-plugins/features/<area>/<capability>/specs/<spec-kind>.md`，用户路径优先；正文、章节标题和表头默认使用中文，代码标识、命令、字段名和状态枚举可保留英文。
...
9. **维护索引**：新增、移动、批准、废弃或拆分规格时，更新 `docs/coding-plugins/INDEX.md`；没有索引则按 `templates/spec-index.md` 创建。
...
路径只表达归属和类型，不表达时间。时间、状态、标签、相关代码和相关规格写入文档 metadata，并同步到 `docs/coding-plugins/INDEX.md`。metadata key 保持英文，中文展示写入正文 `文档信息` 表。
...
```bash
rg -n "<area>|<capability>|<tag>|<Spec ID>|<code path>" docs/coding-plugins/features
```

如果同一 `features/<area>/<capability>/specs/<spec-kind>.md` 已存在，优先更新它；如果新需求横跨多个能力，拆成多个 capability 规格，并在 `related_specs` 和 `INDEX.md` 中互链。
