...
**核心原则：**`spec-driven-development` 是 SDD 文档链路编排入口，不直接承接所有正文写作；需求正文交给 `writing-requirements`，技术方案交给 `writing-technicals`，测试用例交给 `writing-test-cases`，最后才进入 `writing-plans`。
...
8. **编写技术方案**：需求确认后调用 `writing-technicals`，输出 `technicals/<feature-name>-TDD.md`。
9. **编写测试用例**：technical 确认后调用 `writing-test-cases`，输出 `test-cases/<feature-name>-TCD.md`。
...
| 技术方案设计 | `writing-technicals` | `docs/coding-plugins/features/<feature-name>/technicals/<feature-name>-TDD.md` | 架构、关键决策、技术落点 |
...
| TDD 证据 | `test-driven-development` | `docs/coding-plugins/features/<feature-name>/evidences/<feature-name>-TED.md` | 实际 RED/GREEN/REFACTOR 证据 |
