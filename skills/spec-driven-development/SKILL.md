...
6. **编写需求文档**：调用 `writing-requirements`，按场景输出 `docs/coding-plugins/features/<feature-name>/specs/<spec-kind>.md`。
7. **需求校验和确认**：运行 `validate_spec.py`，按 `references/spec-review-checklist.md` 自审，并等待用户确认。
8. **编写技术方案**：需求确认后调用 `writing-technical-design`，输出 `technical/technical-design.md`。
9. **编写测试用例**：technical 确认后调用 `writing-test-cases`，输出 `test-cases/test-cases.md`。
...
| 需求文档 | `writing-requirements` | `docs/coding-plugins/features/<feature-name>/specs/<spec-kind>.md` | 需求、契约、验收、Spec ID |
| 技术方案设计 | `writing-technical-design` | `docs/coding-plugins/features/<feature-name>/technical/technical-design.md` | 架构、关键决策、技术落点 |
| 测试用例文档 | `writing-test-cases` | `docs/coding-plugins/features/<feature-name>/test-cases/test-cases.md` | Spec ID 到测试用例、断言、数据 |
| 实现计划 | `writing-plans` | `docs/coding-plugins/features/<feature-name>/plans/implementation.md` | 任务拆分、命令、执行顺序 |
| TDD 证据 | `test-driven-development` | `docs/coding-plugins/features/<feature-name>/evidence/tdd-evidence.md` | 实际 RED/GREEN/REFACTOR 证据 |
...
如果同一 `features/<feature-name>/specs/<spec-kind>.md` 已存在，优先更新它；如果新需求横跨多个能力，拆成多个 feature 需求文档，并在 `related_specs` 和 `INDEX.md` 中互链。
