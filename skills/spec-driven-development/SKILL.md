...
| 实现计划 | `writing-plans` | `docs/coding-plugins/features/<feature-name>/plans/<doc-id>-IPD.md` | 任务拆分、命令、执行顺序 |
...
PRD 内可以组合 feature、API contract、schema、state machine、acceptance 或 maintenance 章节。不要按规格类型创建多个独立需求文件；只有当同一 feature 下存在多条独立需求链路时，才按 `doc_id` 拆分 PRD。
