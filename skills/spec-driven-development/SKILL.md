...
5. **先读 metadata**：涉及 Coding Plugins 文档关系时先使用 `document-metadata`；读取目标 feature README 和相关 PRD、TDD/TID、TCD、IPD、TED 的 frontmatter；关联关系以 `related_docs`、README `tags` 和 `docs/coding-plugins/document-contract.md` 为准。
...
11. **维护索引和关系**：新增、移动、批准、废弃或拆分文档时，运行 `npm run preflight -- --write-index` 生成 `docs/coding-plugins/INDEX.md`；新增下游 TDD/TID/TCD/IPD/TED 后必须回填 PRD 的 `related_docs` metadata。
...
| 场景 | 文档决策 | 需要的文档 |
| --- | --- | --- |
| 只解释代码、读取状态、运行验证、普通 review、无行为影响的小整理 | 不创建新文档 | 不创建 PRD/TDD/TID/TCD/IPD/TED；只回报结果或使用对应技能 |
| 新功能、用户可见行为、API/SDK/CLI 契约、schema、状态机、兼容迁移、安全/性能/稳定性改造、长期回归修复 | 完整链路 | README + PRD -> TDD -> TID -> TCD -> IPD -> TED |
| demo/sample、一次性验证、纯文档整理、极小配置或文案调整、已有外部验证报告且无长期实现计划 | 轻量例外 | README + PRD + TED；README 的 `## 轻量例外` 必须按 `Doc ID | 规格 ID | 证据` 声明适用范围 |
| 需求已 approved，但尚未执行实现 | 计划链路 | PRD + TDD + TID + TCD + IPD；TED 可以先是 draft 证据目标，不要求最终验证通过 |
| 实现已完成或 TDD/TID 标记 `lifecycle_status: implemented` | 完成链路 | PRD + TDD + TID + TCD + IPD + TED，TED 必须包含最终验证通过的证据 |
