...
4. **检索既有规格**：先查 `docs/coding-plugins/specs/INDEX.md`，再用 `rg` 查 area、capability、tag、Spec ID 和相关代码路径；能更新既有规格时不要新建。
5. **写结构化规格**：按选定模板输出到 `docs/coding-plugins/specs/<area>/<capability>/<spec-kind>.md`，用户路径优先；正文、章节标题和表头默认使用中文，代码标识、命令、字段名和状态枚举可保留英文。
6. **填写 metadata**：复制或补齐 `templates/spec-metadata.md` 字段；机器 key 保持英文稳定，正文用 `## 文档信息` 提供中文摘要；日期写在 `created`/`updated`，不要写进文件名。
7. **分配 Spec ID**：使用 `REQ/API/SCHEMA/STATE/ERR/AC/NFR/MIG/OBS/NON` 前缀。
8. **建立追踪种子**：用 `templates/traceability-matrix.md` 列出每个 Spec ID 的验证方式。
9. **维护索引**：新增、移动、批准、废弃或拆分规格时，更新 `docs/coding-plugins/specs/INDEX.md`；没有索引则按 `templates/spec-index.md` 创建。
10. **运行规格校验**：执行 `python3 skills/spec-driven-development/scripts/validate_spec.py <SPEC_FILE_PATH>`，修复失败项。
11. **规格自审**：按 `references/spec-review-checklist.md` 检查并修复缺口。
12. **用户确认**：要求用户审阅规格。用户要求修改时，改 spec 并重新校验和自审。
13. **过渡计划**：用户确认后调用 `writing-plans`。
...
docs/coding-plugins/specs/billing/invoice/api-contract.md
docs/coding-plugins/specs/search/indexing/schema.md
```

路径只表达归属和类型，不表达时间。时间、状态、标签、相关代码和相关规格写入文档 metadata，并同步到 `docs/coding-plugins/specs/INDEX.md`。metadata key 保持英文，中文展示写入正文 `文档信息` 表。
...
```bash
rg -n "<area>|<capability>|<tag>|<Spec ID>|<code path>" docs/coding-plugins/specs
```

如果同一 `<area>/<capability>/<spec-kind>.md` 已存在，优先更新它；如果新需求横跨多个能力，拆成多个 capability 规格，并在 `related_specs` 和 `INDEX.md` 中互链。
