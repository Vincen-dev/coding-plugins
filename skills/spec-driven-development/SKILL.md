...
6. **编写需求文档**：调用 `writing-requirements`，输出 `docs/coding-plugins/features/<feature-name>/requirements/<feature-name>-PRD.md`。
7. **需求校验和确认**：运行 `validate_spec.py`，按 `references/spec-review-checklist.md` 自审，并等待用户确认。
8. **编写技术文档**：需求确认后调用 `writing-technicals`，输出 `technicals/<feature-name>-TDD.md`，需要模块级落地时同时输出 `technicals/<feature-name>-TID.md`。
9. **编写测试用例**：TDD/TID 确认后调用 `writing-test-cases`，输出 `test-cases/<feature-name>-TCD.md`。
10. **维护索引**：新增、移动、批准、废弃或拆分文档时，运行 `python3 scripts/preflight.py --write-index` 生成 `docs/coding-plugins/INDEX.md`。
11. **过渡计划**：PRD、TDD/TID 和 TCD 都确认后，调用 `writing-plans`。
