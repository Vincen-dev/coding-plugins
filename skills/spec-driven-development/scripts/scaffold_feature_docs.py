...
def render_prd(feature: str, doc_id: str, title: str, status: str, current_date: str, tags: list[str]) -> str:
    ...
    return (
        ...
        "| 编号 | 优先级 | 需求 | 验证方式 |\n"
        "| --- | --- | --- | --- |\n"
        "| REQ-001 | 必须 | 请写明一条可测试需求。 | 请写明验证命令、测试类型或人工验收证据。 |\n\n"
        "## API / SDK / CLI 契约\n\n"
        "不适用：请写明不涉及外部接口契约的原因。\n\n"
        "## Schema / 数据契约\n\n"
        "不适用：请写明不涉及数据结构、配置或事件 payload 的原因。\n\n"
        "## 状态机 / 生命周期\n\n"
        "不适用：请写明不涉及状态迁移或生命周期的原因。\n\n"
        "## 验收标准\n\n"
        "| 编号 | 场景 | 前置条件 | 操作 | 期望结果 |\n"
        "| --- | --- | --- | --- | --- |\n"
        "| AC-001 | 请写明场景名。 | 请写明前置条件。 | 请写明用户或系统动作。 | 请写明可观察结果。 |\n\n"
        "## 维护 / 迁移 / 回归约束\n\n"
        "不适用：请写明不涉及维护、迁移或回归风险的原因。\n\n"
        "## 错误和边界情况\n\n"
        "| 编号 | 条件 | 期望行为 | 验证方式 |\n"
        "| --- | --- | --- | --- |\n"
        ...
        "| --- | --- | --- | --- | --- |\n"
        "| REQ-001 | 请写明验证类型。 | `请写明命令或证据路径` | 请写明任务编号或不适用。 | 计划中 |\n"
    )
