...
| 编写技术方案、技术设计、架构方案、ADR 或 implementation design | `writing-technicals` |
...
| 已有批准规格，需要技术实现方案 | `writing-technicals` |
| 已有批准需求和技术设计，需要测试设计 | `writing-test-cases` |
| 已有批准规格、技术设计和测试用例，需要落地步骤 | `writing-plans` |
...
- 需要从需求进入计划时，先用 `writing-requirements` 写需求文档，再用 `writing-technicals` 写技术方案，再用 `writing-test-cases` 写测试用例文档，最后用 `writing-plans` 拆执行任务。
