...
description: 开始任何对话时使用；兼容原 Superpowers 入口，要求在回复、提问或行动前先检查并调用相关技能。
---
...
# 使用 Coding Plugins / Superpowers 兼容入口
...
2. Coding Plugins / Superpowers 技能。
3. 默认系统行为。
...
- 在 Claude Code 中，插件技能以 `/coding-plugins:<skill-name>` 命名空间出现。Claude Code 是原 Superpowers 的主要目标环境之一，技能中出现的 Claude 工具名可直接使用；需要映射时见 `references/claude-tools.md`。
...
1. 流程技能优先：`brainstorming`、`systematic-debugging` 等决定如何推进。
2. 实现技能其次：TDD、代码评审、worktree、子代理等指导具体执行。

例子：

- “帮我做一个功能” -> 先 `brainstorming`，再 `writing-plans`。
...
`using-superpowers` 保留给原版兼容；中文主入口是 `using-coding-plugins`。两者规则一致。
