...
- **本文结论:** <说明本 TED 要执行哪些任务、最终成功标准和交付边界。>
- **当前状态:** 草稿，等待执行确认。
...
- **Intent Lock:** <一句话锁定本 TED 执行意图，执行阶段不得扩展到其他目标。>
...
- **Rewind Triggers:** 上游 PRD/TSD/TVD 变更、source_hash 不匹配、新增外部行为、接口或 schema 改动、验证失败且影响需求契约。
...
- 技术约束：<用 1 到 3 条列出 TSD 中会影响任务顺序或实现边界的约束。>
- 测试约束：<用 1 到 3 条列出 TVD 中必须执行或补齐的测试。>

不要在本节复述完整技术方案；完整方案以 TSD 为准，测试设计以 TVD 为准。
...
- TASK-001：<前置任务、阻塞条件、是否可并行、完成后解锁哪些任务。>
- 可并行任务：<列出可并行任务；没有时说明所有任务必须串行执行。>
- 任务完成定义：<每个任务完成所需的文件状态、测试命令、评审或 VED 字段。>
...
- 范围漂移：执行中发现新增外部行为、接口、schema、状态或未批准需求。
- 验证失败：RED/GREEN/REFACTOR 或最终验证失败且影响需求契约。
- 环境阻塞：必要依赖、权限、数据或工具不可用，且没有用户批准的替代验证。
