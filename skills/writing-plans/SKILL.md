...
默认保存到：`docs/coding-plugins/features/<area>/<capability>/plans/implementation.md`。用户偏好的路径优先。

对应 TDD Evidence 默认保存到：`docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`。
...
- 计划路径的 `<area>/<capability>` 应和规格路径保持一致，例如 `features/auth/login/specs/feature.md` 对应 `features/auth/login/plans/implementation.md`。
- TDD Evidence 路径的 `<area>/<capability>` 也应保持一致，例如 `features/auth/login/plans/implementation.md` 对应 `features/auth/login/evidence/tdd-evidence.md`。
...
完整技术设计写在 `docs/coding-plugins/features/<area>/<capability>/technical/technical-design.md`，计划必须引用它。计划中的技术方案只保留执行所需快照，重点是把设计拆成可测试任务。

必须覆盖：

- **Technical Design Source**：真实存在的 `technical/technical-design.md` 路径。
...
| 技术设计 | `docs/coding-plugins/features/[area]/[capability]/technical/technical-design.md` |
...
**TDD Evidence Target:** `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`
...
| --- | --- | --- | --- | --- |
| REQ-001 | `tests/path/test_file.py` | `test_specific_behavior` | `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md` / RED-GREEN-Final verification | Task 1 |
...
Write or update `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`. When generating a real plan, replace every bracketed placeholder with the concrete expected evidence for this task:
...
> 计划已完成并保存到 `docs/coding-plugins/features/<area>/<capability>/plans/implementation.md`。有两种执行方式：
>
