...
默认保存到：`docs/coding-plugins/features/<area>/<capability>/implementation.md`。用户偏好的路径优先。
...
完整技术设计写在 `docs/coding-plugins/features/<area>/<capability>/technical-design.md`，计划必须引用它。计划中的技术方案只保留执行所需快照，重点是把设计拆成可测试任务。
...
| 技术设计 | `docs/coding-plugins/features/[area]/[capability]/technical-design.md` |
...
> 计划已完成并保存到 `docs/coding-plugins/features/<area>/<capability>/implementation.md`。有两种执行方式：
>
