...
对应 TDD 证据默认保存到：`docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`。
...
- TDD 证据路径的 `<area>/<capability>` 也应保持一致，例如 `features/auth/login/plans/implementation.md` 对应 `features/auth/login/evidence/tdd-evidence.md`。
...
- **技术设计来源**：真实存在的 `technical/technical-design.md` 路径。
...
**TDD 证据目标:** `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`
...
| --- | --- | --- | --- | --- |
| REQ-001 | `tests/path/test_file.py` | `test_specific_behavior` | `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md` / RED-GREEN-最终验证 | 任务 1 |
...
写入或更新 `docs/coding-plugins/features/<area>/<capability>/evidence/tdd-evidence.md`。生成真实计划时，把每个方括号占位内容替换成该任务的具体预期证据：
