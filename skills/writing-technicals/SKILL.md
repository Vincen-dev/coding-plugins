...
技术文档首先是工程交接材料，不是表格填空。读者应能在 60 秒内从阅读摘要、设计摘要、规格映射和关键决策判断：这次改什么、不改什么、为什么这么落地、风险在哪里、下一步测试怎么接。
...
- TID 缺位：TDD 写了设计，但 TID 没有说明实现模式、实现点、代码落点、数据状态、测试交接和回滚边界。
...
- TDD 关注方案和决策，TID 关注实现拆解；不要让两个文档互相复述。
...
- 是否每个真实 TDD 都能通过关联 TID 找到实现模式和实现点。
