...
方案 B 链路规则：approved PRD 的正式链路必须同时沉淀 TDD 和 TID。TID 不再是可选补充；即使是纯文档或轻量变更，也要创建 `<doc-id>-TID.md`，并在其中说明“不新增代码实现，仅维护文档链路和实现约束”。
...
- **技术实现文档**：approved PRD 链路必须创建或更新 `<doc-id>-TID.md`；涉及代码、schema、接口、状态机或迁移时写模块级实现细节，纯文档或轻量变更也必须写明实现层面“不新增代码变更”的边界。
...
6. 创建或更新 `technicals/<doc-id>-TID.md`，并在 TDD 的实现落地章节引用它；如果没有代码实现，TID 仍需说明“不新增代码实现”的范围边界。
