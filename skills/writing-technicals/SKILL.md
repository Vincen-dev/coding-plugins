...
description: 已有批准规格，需要在实现计划前创建或更新技术设计文档、技术实现文档、ADR 或架构方案时使用。
---
...
`writing-technicals` 把规格契约转成两个稳定工程产物：技术设计文档和技术实现文档。技术设计文档回答“为什么这样设计、边界和关键决策是什么”；技术实现文档回答“具体模块、接口、数据结构和迁移步骤如何落地”。实现计划只负责把这些设计拆成可执行任务。

核心原则：技术文档默认独立保存在 `technicals/`，计划只引用它们并拆任务；不要把完整技术方案埋在 `plans/<feature-name>-IPD.md` 里。
...
`<feature-name>` 必须和 PRD、TCD、IPD、TED 路径一致。
...
- **技术实现文档**：当设计会改动代码、schema、接口、状态机或数据迁移时，必须创建或更新 `<feature-name>-TID.md`；纯文档或极轻量变更可在 TDD 中写明“不需要 TID”。
...
5. 创建或更新 `technicals/<feature-name>-TDD.md`，正文标题默认使用中文，并补齐 `related_specs`、`related_plans`、`related_evidence`；关联关系以 frontmatter 为机器源。
6. 如需要代码、schema、接口、状态机或迁移落地，创建或更新 `technicals/<feature-name>-TID.md`，并在 TDD 的实现落地章节引用它。
...
```text
技术设计已保存到 docs/coding-plugins/features/<feature-name>/technicals/<feature-name>-TDD.md。
技术实现文档已保存到 docs/coding-plugins/features/<feature-name>/technicals/<feature-name>-TID.md。
