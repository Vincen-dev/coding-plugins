...
> 写作门禁：真实文档不得保留任何 `<...>`、`YYYY-MM-DD` 或泛化占位；每个实现点必须能追踪到 Spec ID、TD ID、测试交接和执行交接。
...
<说明实现边界、主要代码落点和不实现的内容。必须明确 implementation_mode 为 code 还是 no_code。>
...
<说明数据结构、状态迁移、缓存、持久化或兼容处理；不涉及则写“不涉及”，并说明原因。>
