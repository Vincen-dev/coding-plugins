...
export function isPathIgnoredByGitignore(root, path) {
...
function parseConfig(root) {
    ...
    try {
        ...
        return { config: parsed && typeof parsed === "object" ? parsed : {}, errors: [] };
    }
    catch (error) {
...
function isArtifactModeValue(value) {
    return value === "tracked" || value === "local" || value === "external";
}
export function resolveArtifactMode(root) {
    ...
    const warnings = [];
    let source = "inferred";
    let mode = existsSync(docsRoot) && !docsIgnored ? "tracked" : "local";
