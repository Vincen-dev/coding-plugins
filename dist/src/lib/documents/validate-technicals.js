...
function markdownSectionByAliases(text, headings) {
...
function isRelativeTo(root, path) {
...
function relatedValuesFromMetadata(text, key) {
...
function approvedSpecFilesForTechnical(root, technicalFile, text) {
...
function requiredSpecIdsFromSpecs(specFiles) {
    const ids = new Set();
...
function technicalDesignCoverageIds(text) {
    const ids = new Set();
...
function validateLifecycleMetadata(root, technicalFile, text) {
...
function validateRequiredSections(root, technicalFile, text) {
...
function validateMustSpecCoverage(root, technicalFile, text) {
...
function validateRelatedMetadata(root, technicalFile, text) {
    const featureContext = featureRootForDocument(root, technicalFile);
    if (!featureContext) {
        return [];
    }
...
function validateMappingTableSchema(root, technicalFile, text) {
...
function decisionIdsFromDesign(text) {
...
function validateDecisionIdReferences(root, technicalFile, text) {
    const section = markdownSectionByAliases(text, ["规格到方案映射", "规格到设计映射"]);
    if (!section) {
        return [];
    }
...
function validateHiddenRequirements(root, technicalFile, text) {
    const errors = [];
...
function genericMappingWarnings(root, technicalFile, text) {
    const section = markdownSectionByAliases(text, ["规格到方案映射", "规格到设计映射"]);
    if (!section) {
        return [];
    }
...
function staleWarnings(root, technicalFile, text) {
    ...
    for (const specFile of approvedSpecFilesForTechnical(root, technicalFile, text)) {
        ...
        if (specUpdated && specUpdated > technicalUpdated) {
            warnings.push(`${relative(root, technicalFile)} is stale technical: ${relative(root, specFile)} updated ${specUpdated} > technical updated ${technicalUpdated}`);
        }
    }
    return warnings;
}
function validateNoUnfinishedTemplateContent(root, documentFile, text) {
    const errors = [];
    let inCodeFence = false;
    text.split(/\r?\n/).forEach((line, index) => {
        const stripped = line.trim();
