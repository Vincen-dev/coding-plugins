...
function isTraceabilityTable(headers) {
    const headerText = headers.join(" | ").toLowerCase();
    const hasSpecId = headerText.includes("spec id") || headerText.includes("规格 id") || headerText.includes("规格id");
    const hasEvidence = ["verification", "test file", "command", "evidence", "验证", "测试文件", "命令", "证据"].some((marker) => headerText.includes(marker));
    return hasSpecId && hasEvidence;
}
function sortedDisplay(values) {
    return `['${[...values].sort().join("', '")}']`;
}
function specIds(text) {
    return new Set(allMatches(SPEC_ID_RE, text).map((match) => match[0]));
}
export function validateSpec(path, strict) {
    const errors = [];
    let warnings = [];
    if (!existsSync(path)) {
            ...
            if (rowIds.size > 0 && !isTraceTable) {
                for (const specId of rowIds) {
                    if (definedIds.has(specId)) {
                        errors.push(`Line ${lineNo}: Duplicate Spec ID definition ${specId} (first defined on line ${definedIds.get(specId)}).`);
                    }
                    else {
...
export function formatTextResults(results) {
