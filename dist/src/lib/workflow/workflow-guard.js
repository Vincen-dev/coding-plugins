...
export function validateExecutionLock(path) {
    if (!existsSync(path)) {
        return [];
    }
...
export function validateExecutionSections(path) {
    if (!existsSync(path)) {
        return [];
    }
...
export function buildNextContext(state, options) {
...
export function checkWorkflowGuard(root, options) {
