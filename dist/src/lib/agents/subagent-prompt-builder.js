...
import { buildBrief } from "../workflow/workflow-brief.js";
import { parseFrontmatter } from "../workflow/workflow-state.js";
...
const PROMPT_KINDS = ["implementer", "spec-reviewer", "code-quality-reviewer"];
const PROMPT_BUDGET_MAX_CHARS = {
    implementer: 12_000,
    "spec-reviewer": 9_000,
...
export function extractTaskSection(planText, task) {
...
export function compactTaskName(title, task) {
...
function buildContextBlock(brief, options) {
    const mustRead = (brief.must_read.length ? brief.must_read : ["-"]).map((path) => `- ${path}`).join("\n");
    const maySkip = (brief.may_skip.length ? brief.may_skip : ["-"]).map((path) => `- ${path}`).join("\n");
    const focusSections = (brief.focus_sections.length ? brief.focus_sections : ["-"]).map((section) => `- ${section}`).join("\n");
...
function buildImplementerPrompt(options) {
...
function buildSpecReviewerPrompt(options) {
...
export function recommendModelTier(kind, options) {
...
function promptBudgetEntry(kind, prompt, taskSection) {
...
function costStrategyEntry(kind, budget, taskSection) {
    return {
...
export function buildPrompts(root, options) {
    ...
    return {
        ...
        cost_strategy: Object.fromEntries(Object.entries(promptBudget).map(([name, budget]) => [name, costStrategyEntry(name, budget, taskSection)])),
        prompt_hashes: Object.fromEntries(Object.entries(prompts).map(([name, prompt]) => [name, sha256Text(prompt)])),
        prompts,
    };
}
export function outputPayloadForKind(payload, kind) {
    if (kind === "all") {
        return payload;
    }
    return {
