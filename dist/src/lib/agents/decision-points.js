export const DECISION_POINTS = [
    ...
    },
    {
        id: "DP-1",
        name: "需求批准",
        trigger: "PRD 需求文档完成，准备进入 TSD 技术方案文档前。",
    ...
    },
    {
        id: "DP-2",
        name: "技术方案批准",
        trigger: "TSD 技术方案文档完成，准备进入 TVD 测试用例前。",
        required_input: "TSD、规格到方案映射、关键决策、实现边界、接口落点和测试策略。",
    ...
    },
    {
        id: "DP-3",
        name: "测试用例批准",
        trigger: "TVD 测试用例完成，准备进入 TED 任务执行文档前。",
        required_input: "TVD、Spec ID 到测试用例映射、断言、测试数据和不可自动化验证说明。",
    ...
    },
    {
        id: "DP-4",
        name: "执行计划批准",
        trigger: "TED 任务执行文档完成，准备进入 worktree 和实现阶段前。",
        required_input: "TED、source_hash、执行锁定区、任务总览、验证命令和 VED 目标。",
    ...
    },
    {
        id: "DP-5",
        name: "TDD 例外或调试升级",
        trigger: "无法先写 RED、连续修复失败、bug 根因不明或需要偏离原 TED。",
    ...
    },
    {
        ...
        required_input: "最新验证命令、输出、规格覆盖、VED 证据和未覆盖风险。",
...
export function getDecisionCatalog(version = "governed-v1") {
    return DECISION_CATALOGS[version].map((point) => ({ ...point, skills: [...point.skills] }));
}
export function allDecisionPoints() {
    return getDecisionCatalog("governed-v1");
}
export function getDecisionPoint(pointId, version = "governed-v1") {
